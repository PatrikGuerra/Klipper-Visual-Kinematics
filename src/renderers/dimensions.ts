...
export function drawRectangularDimensions(draw: DrawContext, bedX0: number, bedY0: number, plateX0: number, plateY0: number): void {
  ...
  if (layers.screwPositions && meshAvailable) drawScrewPositionDimensions(draw, bed);
}

function drawRectDimension(draw: DrawContext, rect: RectDimension): void {
  if (rect.w <= 0 || rect.h <= 0) return;
  drawHorizontalDimension(draw.ctx, draw.map, rect.x, rect.x + rect.w, rect.y + rect.h, rect.horizontalOffset, rect.label, rect.color);
...
function drawScrewPositionDimensions(draw: DrawContext, bed: RectShape): void {
  const color = '#9333ea';
  draw.state.screws.forEach((screw, index) => {
    const p = worldToScreen(draw.map, screw.x, screw.y);
    const dx = screw.x - bed.x;
    const dy = screw.y - bed.y;
...
function drawHorizontalDimension(ctx: CanvasRenderingContext2D, map: CanvasMap, x1: number, x2: number, y: number, offsetPx: number, text: string, color: string): void {
  const left = worldToScreen(map, Math.min(x1, x2), y);
  const right = worldToScreen(map, Math.max(x1, x2), y);
  const lineY = left.y + offsetPx;
